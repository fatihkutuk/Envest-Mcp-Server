"""Dashboard summary, user management, tag links, log parameters, Kepware status
from scada_extended.
"""
...
class ScadaDashboardPack:
    """Dashboard summary, system stats, user management, tag links, log params, Kepware status."""
    ...
    def register(self, mcp: FastMCP, cfg: InstanceConfig) -> None:
        ...
        def _get_scada_summary_impl() -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("SELECT nState, COUNT(*) AS sayi FROM kbindb.node GROUP BY nState ORDER BY nState")
                    node_by_state = list(cur.fetchall())
                    cur.execute("SELECT nType, COUNT(*) AS sayi FROM kbindb.node WHERE nState >= 0 GROUP BY nType ORDER BY sayi DESC")
                    ...
                    cur.execute("SELECT state, COUNT(*) AS sayi FROM kbindb.logparameters GROUP BY state ORDER BY state DESC")
                    log_by_state = list(cur.fetchall())
                    report["log_parametreleri"] = {"toplam": log_total, "aktif": log_active, "duruma_gore": log_by_state}
                    ...
                    try:
                        ...
                        pass
                    report["canli_veriler"] = {"toplam_tag": live_tag, "cihaz_sayisi": live_dev, "server_counter": counter_count}
                    ...
                    try:
                        ...
                        cur.execute(f"SELECT COUNT(*) AS c FROM {ex_db}.channeldevice WHERE statusCode IN ({ac})")
                        ...
                        report["channel_devices"] = {
                            "toplam": cd_total, "calisan": cd_active,
                            "duruma_gore": cd_by_status, "calisan_device_tipine_gore": cd_by_type,
                        }
        ...
        def _get_node_distribution_impl() -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    ...
                    with_tag_link = 0
                    try:
                        cur.execute("SELECT COUNT(DISTINCT srcDid) + COUNT(DISTINCT destDid) AS c FROM kbindb.tag_link")
                        with_tag_link = int((cur.fetchone() or {})["c"] or 0)
                    except Exception:
                        pass
            return {
                "duruma_gore": by_state, "tipe_gore": by_type, "urun_tipine_gore": by_product,
                "bolgeye_gore": by_path, "alarm_tanimli_node_sayisi": with_alarm,
                "log_tanimli_node_sayisi": with_log, "canli_tag_olan_node_sayisi": with_live,
        ...
        def _get_channel_device_statistics_impl() -> Any:
            ...
            return {
                "aktif_db": ex_db, "toplam": total, "calisan": active,
                "calismayan": total - active, "duruma_gore": by_status,
                "device_tipine_gore": by_dt, "client_bazinda": by_client,
                "driver_bazinda": driver_dist, "basarisiz_islemler": failed,
            }
        ...
        def _get_user_permissions_impl(userId: int) -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT id, uFirstName, uLastName, uName, gid FROM users WHERE id = %s", (uid,))
                    user = cur.fetchone()
                    if not user:
                    ...
                    cur.execute(
                        ...
                        WHERE ua.uid = %s ORDER BY n.nName
                        """,
                        (uid,),
                    )
                    ...
                    cur.execute(
                        ...
                        WHERE ga.gid = %s ORDER BY n.nName
                        """,
                        (user["gid"],),
                    )
                    ...
                    cur.execute(
                        ...
                        FROM user_node_rel unr LEFT JOIN node n ON unr.nid = n.id
                        WHERE unr.uid = %s ORDER BY unr.time DESC LIMIT 100
                        """,
                        (uid,),
                    )
                    rel = list(cur.fetchall())
            return {
                "user": user, "bireysel_yetkiler": direct, "bireysel_yetki_sayisi": len(direct),
                "grup_yetkileri": group_perms, "grup_yetki_sayisi": len(group_perms),
                "node_iliskileri": rel, "node_iliski_sayisi": len(rel),
            }
        ...
        def _get_node_permissions_impl(nodeId: int) -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        """
                        SELECT ua.uid, u.uFirstName, u.uLastName, u.uName, ua.yVal, ua.time, 'bireysel' AS yetki_tipi
                        FROM user_auths ua JOIN users u ON ua.uid = u.id
                        WHERE ua.nid = %s ORDER BY u.uFirstName
                        """,
                        (nid,),
                    )
                    direct = list(cur.fetchall())
                    cur.execute(
                        """
                        SELECT ga.gid, g.gName, ga.yVal, ga.time, 'grup' AS yetki_tipi
                        FROM user_group_auths ga JOIN user_groups g ON ga.gid = g.id
                        WHERE ga.nid = %s ORDER BY g.gName
                        """,
                        (nid,),
                    )
        ...
        def _list_tag_links_impl(deviceId: int = 0, limit: int = 100) -> Any:
            ...
            sql = f"""
                SELECT tl.pId, tl.srcDid, tl.srcTagPath, tl.destDid, tl.destTagPath,
                       tl.upInterval, tl.type,
                       CASE tl.type WHEN 0 THEN 'Remote' WHEN 1 THEN 'Local' END AS link_tipi,
                       sn.nName AS kaynak_node, dn.nName AS hedef_node, tl.time, tl.createdTime
                FROM tag_link tl
        ...
        def _list_tag_clones_impl(limit: int = 100) -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    ...
                    for c in clones:
                        cur.execute("SELECT src_tag, dest_tag FROM tag_clone_tags WHERE cid = %s ORDER BY id", (c["id"],))
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        FROM user_groups g LEFT JOIN node n ON g.nid = n.id
                        WHERE g.id = %s
                        """,
                        (gid,),
                    )
        ...
        def _list_users_impl(status: str = "active", company: str = "", city: str = "", limit: int = 25, offset: int = 0) -> Any:
            if not cfg.db:
                raise RuntimeError("DB config is missing for this instance.")
            ...
            sql = f"""
                ...
                {wh} ORDER BY u.id DESC LIMIT %s OFFSET %s
            """
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
            ...
            return {"total": total, "limit": limit, "offset": offset, "count": len(users), "users": users}

        @mcp.tool(name=tool)
        def list_users(status: str = "active", company: str = "", city: str = "", limit: int = 25, offset: int = 0) -> str:
        ...
        def _get_user_impl(userId: int) -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT ua.nid, n.nName, n.nType, ua.yVal FROM user_auths ua LEFT JOIN node n ON ua.nid = n.id WHERE ua.uid = %s",
                        (uid,),
                    )
                    user = dict(user)
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        SELECT u.id, u.uFirstName, u.uLastName, u.uName, u.uTel, u.uMail,
                               ...
                               u.lastLogin
                        FROM users u LEFT JOIN user_groups g ON u.gid = g.id
                        WHERE u.uFirstName LIKE %s OR u.uLastName LIKE %s
                           OR u.uName LIKE %s OR u.uTel LIKE %s
                           OR u.uMail LIKE %s OR u.uTitle LIKE %s
                        ORDER BY u.lastLogin DESC LIMIT %s
                        """,
        ...
        def _get_user_stats_impl() -> Any:
            ...
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("SELECT COUNT(*) AS c FROM users WHERE lastLogin >= DATE_SUB(NOW(), INTERVAL 7 DAY)")
                    r7 = int((cur.fetchone() or {})["c"] or 0)
                    cur.execute("SELECT COUNT(*) AS c FROM users WHERE lastLogin >= DATE_SUB(NOW(), INTERVAL 30 DAY)")
                    r30 = int((cur.fetchone() or {})["c"] or 0)
            return {"genel": general, "son_7_gun_giris": r7, "son_30_gun_giris": r30, "sirket_dagilimi": by_company, "grup_dagilimi": by_group}
        ...
        def _list_log_params_impl(nodeId: int = 0, state: int = 1, limit: int = 100, offset: int = 0) -> Any:
            ...
            sql = f"""
                ...
                FROM logparameters lp LEFT JOIN node n ON lp.nid = n.id
                {wh} ORDER BY lp.id DESC LIMIT %s OFFSET %s
            """
            with dbmod.connect(cfg.db) as conn:
                with conn.cursor() as cur:
                    cur.execute(sql, (*params, limit, offset))
        ...
        return [
            {
                ...
                "tools": [p + "get_scada_summary", p + "get_node_distribution", p + "get_channel_device_statistics"],
            },
            {
                "id": "admin_users",
                "title_tr": "Kullanıcı / yetki",
                "tools": [
                    p + "get_user_permissions", p + "get_node_permissions",
                    p + "list_user_groups", p + "get_group_members",
                    p + "list_users", p + "get_user", p + "search_users", p + "get_user_stats",
                ],
            },
            {
                "id": "tags_meta",
