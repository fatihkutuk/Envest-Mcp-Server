---
name: nview-a-kuyu-envest-drenaj
description: |
  nView 'a-kuyu-envest-drenaj' için GENEL.phtml'den otomatik çıkarılmış tag/birim/etiket eşlemesi.
  Use when: node.nView == "a-kuyu-envest-drenaj" ve tag anlamı, birim, rolü soruluyorsa.
  Keywords: BasincSensoru, Debimetre, StatikSeviye, SuSeviye, a-kuyu-envest-drenaj.
version: "1.0.0"
generated: true
source: //10.10.10.72/public/dev.korubin/app/views/point/display/common/a-kuyu-envest-drenaj/GENEL.phtml
---

# nView: a-kuyu-envest-drenaj
...
| `T_CalismaSayac` | Toplam sayaç (su, elektrik, çalışma, şalt) | h | 2 | counter | langt=Çalışma Sayacı; (col) |
| `T_ElektrikKalan` | Toplam sayaç (su, elektrik, çalışma, şalt) | kW/h | 2 | counter | langt=Kalan Elektrik Miktarı; (col) |
| `T_ElektrikReakInduSayac` | Toplam sayaç (su, elektrik, çalışma, şalt) | kVAR | 2 | counter | langt=Reaktif; (col) |
| `T_ElektrikSayac` | Toplam sayaç (su, elektrik, çalışma, şalt) | kW/h | 2 | counter | langt=Elektrik Sayacı; (col) |
| `T_PompSaltSayisi` | Toplam sayaç (su, elektrik, çalışma, şalt) | adet | 0 | counter | langt=Şalt Sayısı; (col) |
| `T_SuKalan` | Toplam sayaç (su, elektrik, çalışma, şalt) | m³ | 2 | counter | langt=Kalan Su Miktarı; (col) |
| `T_SuSayac` | Toplam sayaç (su, elektrik, çalışma, şalt) | m³ | 2 | counter | langt=Su Sayacı; (col) |
...
| `DrenajSeviye` |  | m | 2 | unknown |  |
| `eNetGuc` | [langt:P1 Güç] | kW | 2 | unknown | (col) |
| `hidrolikVerim` | [langt:Hidrolik Verim] | % | 2 | unknown | (col) |
| `m3Maliyet` | [langt:Birim Maliyeti] | ₺/m³ | 2 | unknown | (col) |
| `MotorSicaklik` | Motor sıcaklığı | °C | 2 | measurement | langt=Motor; (col) |
| `NPSHSeviye` | NPSH seviye | m | 2 | measurement | langt=NPSH Seviye |
| `P1_Verim` | Pompa bazlı ölçüm | kWh | 2 | pump_measurement | langt=P1 Verim; (col) |
| `PanoSicaklik` | Pano sıcaklığı | °C | 2 | measurement | langt=Pano; (col) |
| `sistemVerim` | [langt:Sistem Verim] | % | 2 | unknown | (col) |
| `SuSicaklik` | Su sıcaklığı | °C | 2 | measurement | langt=Su; (col) |
| `toplamHm` | Toplam basma yüksekliği (alias) | m | 2 | measurement | langt=Toplam hm; (col) |
| `txt_CikisDepo` |  |  | 2 | unknown |  |
| `xGunMaliyet` | [langt:Toplam Maliyet] | TL<b></b> | 2 | unknown | (col) |
...
- **unknown**: `Sulama_Basladi`, `XVN_ACTAktif`, `YaklasikHidrolikVerim`, `length`
...
| `calismamod` | [langt:Sabitleme] | [langt:Çalışma Ayarları] | `XC_SabitModBasinc`, `XC_SabitModDebi`, `XC_SabitModSeviye`, `XC_SabitModSeviye2`, `XC_SabitModGuc`, `XC_SabitModFrekans` |
| `sistem` | [langt:Sistem] | [langt:Sistem Ayarları] | `XS_StatikSevGuncSure`, `XD_BasmaYukseklik` |
| `pid` | [langt:PID] | [langt:PID Ayarları] | `XC_BasincPIDKP`, `XC_BasincPIDKI`, `XC_BasincPIDKD`, `XC_BasincPIDKT`, `XC_DebiPIDKP`, `XC_DebiPIDKI`, `XC_DebiPIDKD`, `XC_DebiPIDKT` …+12 |
| `sensor` | [langt:Sensör] | [langt:Sensör Ayarları] | `XS_BasincSensoruMax`, `XS_BasincSensoruKalibre`, `BasincSensoru`, `XS_HatBasincMax`, `XS_HatBasincKalibre`, `BasincSensoru2`, `XS_SeviyeMax`, `XS_SeviyeKalibre` …+15 |
...
- **Debi**: `Debimetre`
- **Seviye / uzunluk (m/cm)**: `CikisDepoSeviye`, `DrenajSeviye`, `NPSHSeviye`, `StatikSeviye`, `SuSeviye`, `XD_CikisDepoYukseklik`, `toplamHm`
